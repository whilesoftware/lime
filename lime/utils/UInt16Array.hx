...
@:forward() @:generic @:arrayAccess abstract UInt16Array(ArrayBufferView) from ArrayBufferView to ArrayBufferView {
	...
	public function subarray (begin:Int, end:Null<Int> = null):UInt16Array {
		
		if (end == null) end == this.length;
		return new UInt16Array (this.buffer, begin << 1, end - begin);
	...
	@:noCompletion @:arrayAccess public inline function __get (index:Int):Null<UInt> {
		...
		#if cpp
		untyped return __global__.__hxcpp_memory_get_ui16 (this.buffer.getData (), index + this.byteOffset) & 0xffff;
		#else
		this.buffer.position = index + byteOffset;
	...
	@:noCompletion @:arrayAccess public inline function __set (index:Int, value:UInt):UInt {
		...
		#if cpp
		untyped __global__.__hxcpp_memory_set_ui16 (this.buffer.getData (), index + this.byteOffset, value);
		#else
